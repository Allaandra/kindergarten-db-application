...
const getGroups = async (req, res) => {
    ...
    // Ми беремо всі дані групи + формуємо красиве ім'я вихователя
    const sql = `
        SELECT 
            g.id, 
            g.name, 
            g.age_category, 
            g.max_capacity,
            g.educator_id, -- Цей ID потрібен для редагування (щоб форма знала, кого вибрати), але в таблиці ми його сховаємо
            
            -- Якщо вихователь є, пишемо ПІБ. Якщо ні - "Не призначено"
            CASE 
...
const getChildren = async (req, res) => {
    const { auth } = req.body;
    const sql = "SELECT * FROM view_child_full_info"; // Используем VIEW для удобства
